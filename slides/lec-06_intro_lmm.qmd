...
* Helps limit problem of overfitting many different parameters
...
- Fixed part:
    - descrtibe deterministic processes.
...
1. Observations are not independent (**Pseudoreplication**)
...
$$
Y_{ij} = \underbrace{a + \sum_k^n b_k X_{k_{ij}}}\text{fixed} + \underbrace{\epsilon_{ij} + a_j}_\text{random}
$$
...
$$Richness = a + b_1\ NAP + b_2\ Exposure + \epsilon$$

::: {.content-visible when-format="revealjs"}
. . .
:::
 Adding index for easier explanation, `i` -> observation
...
## BLUPs ($a_j$) are shrinked

- Estimates for each groups are constrained (shrinked) too avoid extreme values.
...
- We estimated the varaince explained by the model

${\sigma_{\hat{y}}}^2$.

- We decomposed teh residual variance in 2 :
    + ${\sigma_{a_j}}^2$ : across beach variance
    + ${\sigma_{\epsilon}}^2$ Variation not associeated to beaches (*i.e.* within-beach variance)

## Estimating repeatability

- **Repeatability** Proportion of (residual) variance associated to among levels of a random effect.

- Here variation associted to among beach differences
...
$$ r = \frac{{\sigma_{a_j}}^2}{{\sigma_{a_j}}^2 + {\sigma_{\epsilon}}^2}$$
...
## Repeatability is conditionned on fixed effects

Differences among groups can be explained by fixed effects and thus influence ${\sigma_{a_j}}^2$ 
...
$$
r\ (without\ exposure) = \frac{`r round(summary(mod.mix.avec)$varcor$Beach[1,1], 3)`}{`r round(summary(mod.mix.avec)$varcor$Beach[1,1],3)` + `r round(summary(mod.mix.avec)$sigma^2, 2)`} = `r paste(round(summary(mod.mix.avec)$varcor$Beach[1,1] / (summary(mod.mix.avec)$varcor$Beach[1,1] + summary(mod.mix.avec)$sigma^2), 2))`
$$
...
- Complex model use only when necessaryCes modèles sont compliqués, utilisez-les seulement lorsqu'ils sont nécessaires.

- Easy to build a nonsensical models which is overparametrized ...
...
$$
Richness_{ij} = \underbrace{a + b_1{NAP_{ij}} + b_2{Exposure_j}}_\text{fixed} + \underbrace{a_j + \color{#DD3333}{b_{2j}NAP_{ij}} + \epsilon_{ij}}_\text{random}
$$
...
$$\epsilon_{ij} \sim N(0, \sigma_{a_j})$$
...
\sim N\left(0,
\begin{matrix}
\sigma_{a_j}^2  & \color{blue}{cov(a_j, b_{2j})} \\
\color{blue}{cov(a_j, b_{2j})} & \color{#DD3333}{\sigma_{b_{2j}}^2}
\end{matrix}
\right)
$$
...
$$
Richness_{ij} = \underbrace{a + b_1{NAP_{ij}} + b_2{Exposure_j}}_\text{fixed} + \underbrace{\color{#DD3333}{a_j + b_1iNAP_{ij}} + \epsilon_{ij}}_\text{random}
$$
...
### Fixed effects
  ...
  a. based on question 
  b. based on counfounding effects (think and justify based on causality)
